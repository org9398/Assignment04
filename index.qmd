...
We got our data from the U.S. Census Bureau's Household Pulse Survey - linked [here.](https://www2.census.gov/programs-surveys/demo/datasets/hhp/)

We specifically used data from Week 52.

**Graph 1**

race vs educational attainment

```{r}
pulse %>%
  ggplot() +
  geom_bar(mapping = aes(x = rrace), show.legend = TRUE) +
  facet_wrap(~ eeduc)
```
...
**Graph 3 - need to figure out**

```{r}
pulse %>%
  ggplot() +
  geom_point(mapping = aes(x = trentamt, y = tccarecost)) 
...
continuous variables - rent, cost of childcare, household spent on food
